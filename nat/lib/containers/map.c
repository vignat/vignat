...
/*@
  lemma void buckets_ok_acc_bounded_rec<kt>(list<pair<kt, nat> > acc, list<bucket<kt> > buckets, int bound)
...
/*@
          ...
          get_wraparound(get_wraparound(nil, buckets), buckets);
  {
    ...
    //any bounded by length(buckets) acc wont make a difference.
  }
  @*/
