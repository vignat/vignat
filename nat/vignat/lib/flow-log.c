...
void log_int_key(const struct int_key *key) {
    NAT_DEBUG( "{int_src_port: %d(%d); dst_port: %d(%d);\n"
         " int_src_ip: ",
    ...
    NAT_DEBUG( "; dst_ip: ");
    log_ip(key->dst_ip);
    NAT_DEBUG(" int_device_id: %d; protocol: %d}",
            key->int_device_id, key->protocol);
}

void log_ext_key(const struct ext_key *key) {
    NAT_DEBUG( "{ext_src_port: %d(%d); dst_port: %d(%d);\n"
         " ext_src_ip: ",
    ...
    NAT_DEBUG( "; dst_ip: ");
    log_ip(key->dst_ip);
    NAT_DEBUG(" ext_device_id: %d; protocol: %d}",
            key->ext_device_id, key->protocol);
}

void log_flow(const struct flow *f) {
    NAT_DEBUG( "{int_src_port: %d(%d); ext_src_port: %d(%d);\n"
    ...
    NAT_DEBUG( "; dst_ip: ");
    log_ip(f->dst_ip);
    NAT_DEBUG( " int_device_id: %d; ext_device_id: %d;\n"
            " protocol: %d}",
