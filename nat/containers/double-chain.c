...
int dchain_allocate(int index_range) {
    if (NULL == (cells = malloc(sizeof(struct dchain_cell)*
        ...
        return 0;
    dchain_imp_init(cells, index_range);
    return 1;
}

int dchain_allocate_new_index(int *index) {
    return dchain_imp_allocate_new_index(cells, index);
}

int dchain_free_index(int index) {
    return dchain_imp_free_index(cells, index);
}

int dchain_get_oldest_index(int *index) {
    return dchain_imp_get_oldest_index(cells, index);
}

int dchain_rejuvenate_index(int index) {
    return dchain_imp_rejuvenate_index(cells, index);
}
