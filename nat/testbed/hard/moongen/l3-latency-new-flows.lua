...
function master(args)
	...
	for _,nflws in pairs({1,10,100,1000,10000,20000,30000,40000,50000,60000,63999}) do
		-- Heatup phase
		...
		local timerTask = mg.startTask("timerSlave", txDev:getTxQueue(1), rxDev:getRxQueue(1), args.size, args.timeout)
...
function timerSlave(txQueue, rxQueue, size, duration)
	if size < 84 then
	...
	local hist = hist:new()
	local counter = 0
	local rateLimit = timer:new(11/flows)
	local baseIP = parseIPAddress(SRC_IP_BASE)
