...
static struct flow* get_flow_int(struct int_key* key) {
    ...
    if (like_hash.has_next_key) {
        assert(!like_hash.initialized_sample);
...
main_loop(__attribute__((unused)) void *dummy)
{
    ...
    //while (1) --- do only one step. single iteration.
    {
