...
/*@ ensures result == 0 ?
              *map_out |-> old_mo :
              (*map_out |-> ?new_mo &*&
               result == 1 &*&
               mapp<t>(new_mo, kp, hsh, mapc(capacity, nil))); @*/

int map_get/*@ <t> @*/(struct Map* map, void* key, int* value_out);
/*@ requires mapp<t>(map, ?kp, ?hsh, mapc(?capacity, ?contents)) &*&
             kp(key, ?k) &*&
             *value_out |-> ?old_v; @*/
/*@ ensures mapp<t>(map, kp, hsh, mapc(capacity, contents)) &*&
            kp(key, k) &*&
...
/*@ ensures mapp<t>(map, kp, hsh, mapc(capacity, map_put_fp(contents, k, value))); @*/
...
/*@ ensures mapp<t>(map, kp, hsh, mapc(capacity, map_erase_fp(contents, k))) &*&
            [0.5]kp(key, k) &*&
            *trash |-> ?k_out &*&
            [0.5]kp(k_out, k); @*/
