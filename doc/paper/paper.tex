...
iterations of a loop in a single one. To support this kind of generalization, we
added automatic havoc-ing~\cite{barnett2005boogie} to KLEE. Havoc-ing simply
replaces all the variables that may change in the loop by unconstrained symbols
at the beginning of the iteration. In our example that would be the variables
\code{begin} and \code{queue_len} and the contents of \code{packets[0..CAP]},
but not the value of the \code{packets} pointer because it never changes. The
step 1 enables Vigor to reduce each problematic loop to a single generalized
...
the party here. When indexing an array with a symbolic variable (or when
...
value of \code{CAP}. The fact that the code does not crash on any path amounts
to a proof of crash freedom. Or does it? The validity of the proof depends on
our library implementation of \code{ring} not crashing; and the symbolic model
of \code{ring} accurately capturing how the \code{ring} implementation works.
...
\end{lstlisting}
  \caption{A symbolic model for \code{ring_pop_front}. It fills symbolically a
    packet and adds a constraint to it.}
...
function. This happens in the last line on the listing. The assert contains the
...
packet from the ring, it satisfies the \code{VIGOR_CHECK} (Fig.~\ref{lst:vigor}
l.\ref{line:vigcheck}).
...
\subsection{Detecting model inconsistencies}

The component developer produces a number of model that capture the component
...
overapproximate and underapproximate models.
