...
performance is on par with other NATs. %\footnote{Disclaimer: we use acronym NAT in this paper to refer to a more narrow kind - Dynamic Source NAPT (network address-port translation~\cite{rfc3022}).}. 
...
\begin{enumerate}
    ...
    \item \(Fi\): The function input (\(Fi = fun\_name + args + arg\_pointee\_vals\_before + constraints\)) consists of the function arguments (possibly symbolic), and constraints bounding these arguments (specifically, the transitive closure of all the constraints involving the symbols mentioned in the formulas for the arguments). For a pointer argument (which must point to a concrete address --- practical limitation), the pointee must be considered according to the \S\ref{sec:handling-pointers}. We use the numerical value of the pointer to detect aliasing between the variables, and then throw it away, since we forbid pointer comparison anyway.
