...
Our NF accumulates state in the ring thus making many existing verification
...
replaced the infinite loop with a single iteration we have to assume (due to
...
Let us follow now the `pord != 9' condition across the framework. During
exhaustive SE, SEE first learns the constraint in the \code{if} condition:
Fig.~\ref{lst:vigor} l.\ref{line:if-cond}. It however forgets it on the next
line, because our model of \code{ring_push_back} discards the packet being
pushed. However, this constraint arises again on the call to
\code{ring_pop_front}(l.\ref{line:pop-front}) as
\code{packet_constraints}(Fig.~\ref{lst:customizations},l.\ref{line:c-pkt-cnstr}),
according to the symbolic model - Fig.~\ref{lst:symbolic-model}. Being applied
to the fresh packet from the ring, it satisfies the \code{VIGOR_CHECK}
