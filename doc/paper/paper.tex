...
would like to verify --- that we do not forward packets with port 9.
...
``library of components'' (e.g. the ring in the step 2). This works well for NF
...
It starts with \code{loop_invariant_produce} that is inserted automatically at
the beginning of a loop iteration. It continues then with calls to
...
\end{lstlisting}
  \caption{A call sequence, produced by SEE. Vigor adds formal annotations to
    constrain the execution space. Unexplained
    lines(\ref{line:unexplained1},\ref{line:unexplained2},\ref{line:unexplained3},\ref{line:unexplained4})
    are the automatically inserted helper steps to guide the proof checker.}
...
\subsection{Model inconsystency symptoms}
