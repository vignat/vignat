...
\code{VIGOR_CHECK} adds additional property that we would like to verify ---
...
logic, and C) due to the limitations of our prototype.

{\bf Step 4: Run Vigor checker.} Vigor will find the appropriate symbolic model,
check that the application does not crash in any execution scenario. It will
...
automatic havoc-ing~\cite{barnett2005boogie} (replace all variables changed in
the loop with unconstrained symbols). The step 1 enables Vigor to reduce each
problematic loop to a single generalized iteration that allows reasoning in one
pass.
...
\end{lstlisting}
  \caption{A symbolic model for \code{ring_pop_front}.}
...
iteration. It continues then with calls to \code{ring_full} \code{ring_empty},
...
\end{lstlisting}
  \caption{An overapproximate ring model fragment.}
...
\end{lstlisting}
  \caption{An underapproximate ring model fragment.}
