...
Locate force_signed_int.
  
SearchPattern (int -> Z).

Definition guess_sqrt_spec :=
 DECLARE _guess_sqrt
  WITH sh : share, n : Z, guess : Z, vn : val, vguess : val, rv : val, r : Z
  PRE [_n OF tuint, _guess OF tuint ]
      PROP(repr n vn ; repr guess vguess ; repr r rv; 0 <= n < Int.modulus;
  ...
  LOCAL ()
  SEP().
(*(`(eq (Vint (Int.repr (guess_sqrtZ n guess)))) retval).*)
...
Proof.
  start_function.
  destruct H2 as [A0 AM].
  rename H3 into B0, H4 into BM.
  name narg _n.
