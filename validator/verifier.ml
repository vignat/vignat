...
let verify_ir ir verifast fname outf proj_root lino_fname =
  ...
  let _ = Sys.command (verifast ^ " -c -prover z3v4.5 -I " ^ proj_root ^ " -I $RTE_SDK/$RTE_TARGET/include" ^
                       " " ^ fname ^ " > " ^ outf)
  in
...
let export_assumptions
  ...
  let export_lino = String.strip (In_channel.read_all lino_fname) in
  let _ =
    Sys.command ( verifast ^ " -c -prover z3v4.5 -I " ^ proj_root ^ " -I $RTE_SDK/$RTE_TARGET/include" ^
